#!/usr/bin/env python3
"""只 submit 不 poll, 拿到 job_id 立即退出. 用于跨 sandbox 持久化进度.

用法:
  python3 submit_only.py <TAG> [<TAG> ...]
"""
import json
import sys
import time
...
from gladia import load_keys, KeyRotator, upload, transcribe  # noqa
...
def main():
    tags = sys.argv[1:]
    if not tags:
        sys.exit("usage: submit_only.py TAG [TAG ...]")
